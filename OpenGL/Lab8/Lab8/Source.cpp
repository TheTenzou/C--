...
void display(void)
{
	...
	glutSolidCube(1.0); // рисуем шар

	glMaterialfv(GL_FRONT, GL_AMBIENT, mat1_amb);
	glMaterialfv(GL_FRONT, GL_DIFFUSE, mat1_dif);
	glMaterialfv(GL_FRONT, GL_SPECULAR, mat1_spec);
	glMaterialf(GL_FRONT, GL_SHININESS, mat1_shininess);
	glTranslatef(-1.5, -10.0, -10); // смешение шара
	//glutSolidCube(10.0); // рисуем шар
	glPopMatrix();//возврашение предыдушего значения матрицы
	...
	glPushMatrix();//Сохранение ткошего состояния матрицы
	glTranslatef(0.0, -0.5, 0.7); //смешение конуса
	glRotatef(270.0, 1.0, 0.0, 0.0); //поворот конуса
	glutSolidCone(1.0, 2.0, 50, 50); //рисуем конус
	glPopMatrix();//возврашение предыдушего значения матрицы
...
int main(int argc, char** argv)
{
	...
	glutInitDisplayMode(GLUT_SINGLE | GLUT_RGBA | GLUT_DEPTH); //Инициализация дисплея 
