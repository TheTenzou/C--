...
int input(string fileName, float ** &A, float * &S, int &n)
{
	ifstream file;
	...
	if (file.is_open()) // если файл открылся то читаем
	{
		...
		while (getline(file, line))
		{
			...
			S[i] = stof(line.substr(posl + 2, line.find(' ', posl + 2) - 1));
...
void solve(float** L, float** U, float * B, float* ans, int n)
{
...
int main()
{
	...
	string filename;
	cin >> filename;
	float** A, ** L{ 0 }, ** U{ 0 }, ** R{ 0 }, * sv, * ans;
	int n;
	if (input(filename, A, sv, n))
	{
		cout << "====" << endl;
		for (int i = 0; i < n; ++i)
		{
